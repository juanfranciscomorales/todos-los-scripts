...
## # Deep Learning con deepnet - PARA 2 CLASES ##
...
training$clase <- as.factor(make.names(training$clase)) ## hago que la columna clase sea como factor y con nombres validos para poder hacer que el Deep Learning sea clasificatorio
...
dnnGrid <-  expand.grid(  ## con esto lo que voy a hacer es decir el barrido que va a hacer la funcion para optimizar los siguientes parámetros de dnn
        
        size = 0:10 , ## number of units in the hidden layer. Can be zero if there are skip-layer units
        
        decay = c(0 , 0.0001 , 0.001 , 0.01 , 0.1) ## parameter for weight decay
        
)

### Entreno el modelo por dnn y optimizo los valores


ptm <- proc.time()

dnnfit <- train(clase ~ .,## uso la funcion train del paquete caret para hacer knn. en esta linea especifico cual es el valor a predecir y cuales son las variables independientes. 
                 
                 data = training, ## le digo cuales son mis datos para armar el modelo
                 
                 method = "dnn", ## aca le digo que use knn para armar el modelo
                 
                 trControl = ctrl,  ## le digo que use el elemento ctrl para optimizar el modelo
                 
                 tuneGrid = dnnGrid , ## hago pasar el elemento dnnGrid para probar y encontrar cuales son los valores optimos 
                 
                 linout = FALSE, ## esto lo hago para que sea clasificatorio
                 
                 MaxNWts = 100000 # The maximum allowable number of weights. There is no intrinsic limit in the code, but increasing MaxNWts will probably allow fits that are very slow and time-consuming.
                 
)

## pongo el valor de MaxNWts = 10000 porque sino me tira error y no me deja hacer la corrida

proc.time() - ptm


dnnfit ## imprimo el resultado

plot(dnnfit) ## grafico los resultados del cross validation



variables.importantes <- head(summary(dnnfit), n = 20) ## extraigo un data frame que contiene los valores de influencia y el nombre de las variables. con n decido cuantas variables mas importantes extraigo
...
ggplot(data = variables.importantes , aes(x = var, y = rel.inf)) + geom_bar(stat="identity", fill="steelblue")  + coord_flip() + theme_minimal() + labs(title = "20 most influence variables in Deep Learning", y = "Relative Influence" , x = "Variable") + theme(plot.title = element_text(hjust = 0.5))  ## con esto grafico las 20 variables que mas influyen en el armado del modelo

library(pROC) ## abro el paquete pROC para hacer las curvas ROC

predicciones.train <- predict(dnnfit, newdata = training , type = "prob" , na.action = na.pass) ## hago la prediccion en el training set obteniendo los resultados como probabilidad
...
predicciones.test <- predict(dnnfit, newdata = test, type="prob" , na.action = na.pass)  ## predicciones en el test set expresadas como probabilidad
...
resultado.dnn <- list("Modelo armado por Deep Learning", dnnfit  , "AUC ROC Training" , auc.training, "AUC ROC Test", auc.test) ## armo una lista con todos los resultados que quiero que se impriman
...
test <- "S-M-H test set.csv"  ### nombre del test set
...
### en este caso no lo hice porque no tiene lógica de hacerlo
...
predicciones.dude <- predict(dnnfit, newdata = dude, type="prob" , na.action = na.pass)  ## predicciones en el test set expresadas como probabilidad
...
predicciones.base.datos <- predict(dnnfit, newdata = df.base.datos, type="prob" , na.action = na.pass)  ## predicciones en la base de datos expresadas como probabilidad
...
write.xlsx(x= predicciones.base.datos, file= "Screening por Deep Learning- dnn con caret.xlsx" , colNames= TRUE, keepNA=TRUE) # funcion para guardar los resultados del screening en la base de datos
