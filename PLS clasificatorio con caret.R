...
plsfit <- train(clase ~ .,## uso la funcion train del paquete caret para hacer knn. en esta linea especifico cual es el valor a predecir y cuales son las variables independientes. 
                ...
                method = "pls", ## aca le digo que use Support vector machine para armar el modelo
                
                trControl = ctrl,  ## le digo que use el elemento ctrl para optimizar el modelo
                
                tuneGrid = plsGrid , ## hago pasar el elemento svmGrid para probar y encontrar cuales son los valores optimos 
                
                preProcess = c("center","scale") ) ## aca le digo que me centre y me escale los datos antes de armar el modelo


plsfit ## imprimo el resultado
...
predicciones.train <- predict(object = plsfit, newdata = training, type ="prob" , na.action = na.pass) ## predicciones en el training set expresadas como probabilidad
...
predicciones.dude <- predict(object = plsfit, newdata = dude, type = "prob" , na.action = na.pass) ### predigo en el test set
...
predicciones.base.datos <- predict(object = plsfit, newdata = df.base.datos, type = "prob" , na.action = na.pass) ### predigo en el test set
...
write.xlsx(x= predicciones.base.datos, file= "Screening por PLS clasificatorio con caret.xlsx" , colNames= TRUE, keepNA=TRUE) # funcion para guardar los resultados del screening en la base de datos
