...
y.randomization.lm <- function ( conjunto , iteraciones = 10 , punto.corte=0.05 , steps=6) {
        ...
        for(i in 1:k) {
                ...
                model <- forward.stepwise.testF.lm ( train , punto.corte, steps)#armo el nuevo modelo partiendo del conjunto de descriptores
...
resultados.y.randomization.lm <- lapply(lista.conjuntos2 , y.randomization.lm , iteraciones = 10 , punto.corte=0.05 , steps=9)# aplico a cada conjunto la función llamada y.randomization para hacer la validacion interna y obtengo los intervalos de confianza
