...
test$clase <- as.factor(make.names(test$clase)) # hago que la clase sea factor 
...
#       PRIMER BUSQUEDA EXAUSTIVA EXPLORATORIA DE LOS VALORES DE LOS
...
adaboostGrid <-  expand.grid (  ## con esto lo que voy a hacer es decir el barrido que va a hacer la funcion para optimizar los siguientes parámetros de adaboost
                        
                        nIter = seq(from =10 , to =80 , by =10 ) , ### con esto le digo la cantidad de arboles donde tiene que explorar que está el valor optimo
                        
                        method = c("Adaboost.M1")   ### pruebo los dos metodos de Ada boosting que permite el paquete al que llama CARET que es fastAdaboost
)


### Entreno el modelo por adaboost del paquete fastAdaboost y optimizo los valores


ptm <- proc.time()

adaboostfit <- train(clase ~ .,## uso la funcion train del paquete caret para hacer knn. en esta linea especifico cual es el valor a predecir y cuales son las variables independientes. 
                
                data = training, ## le digo cuales son mis datos para armar el modelo
                
                method = "adaboost", ## aca le digo que use knn para armar el modelo
                
                trControl = ctrl,  ## le digo que use el elemento ctrl para optimizar el modelo
                
                tuneGrid = adaboostGrid ) ## hago pasar el elemento adaboostGrid para probar y encontrar cuales son los valores optimos 
...
adaboostfit ## imprimo el resultado

plot(adaboostfit) ## grafico los resultados del cross validation

print(adaboostfit$finalModel)
...
predicciones.train <- predict(adaboostfit, newdata = training , type = "prob" , na.action = na.pass) ## hago la prediccion en el training set obteniendo los resultados como probabilidad
...
predicciones.test <- predict(adaboostfit, newdata = test, type="prob" , na.action = na.pass)  ## predicciones en el test set expresadas como probabilidad
...
resultado.adaboost <- list("Modelo armado por Boosting", adaboostfit  , "AUC ROC Training" , auc.training, "AUC ROC Test", auc.test) ## armo una lista con todos los resultados que quiero que se impriman
...
adaboostGrid <-  expand.grid (  ## con esto lo que voy a hacer es decir el barrido que va a hacer la funcion para optimizar los siguientes parámetros de adaboost
        ...
        nIter = seq(from = adaboostfit$finalModel$tuneValue$nIter - 9 , to = adaboostfit$finalModel$tuneValue$nIter + 9 , by = 1 ) , ### con esto le digo la cantidad de arboles donde tiene que explorar que está el valor optimo
...
### Entreno el modelo por adaboost y optimizo los valores
...
resultado.adaboost <- list("Modelo armado por Boosting", adaboostfit  , "AUC ROC Training" , auc.training,"Resultado K-fold CV" , getTrainPerf(adaboostfit) , "AUC ROC Test", auc.test) ## armo una lista con todos los resultados que quiero que se impriman
...
predicciones.train <- predict(adaboostfit, newdata = training, type="prob" , na.action = na.pass)  ## predicciones en el train set expresadas como probabilidad
...
saveRDS(adaboostfit, "adaboostfit.rds") ## guardo el modelo 


adaboostfit <- readRDS("adaboostfit.rds") ## vuelvo a cargar el modelo
...
dude <- as.data.frame(fread(input = dude.set, check.names = TRUE)) #leo el archivo con mis descriptores del test set

dude$clase <- as.factor(make.names(dude$clase)) # hago que la clase sea factor 
...
predicciones.dude <- predict(adaboostfit, newdata = dude, type="prob" , na.action = na.pass)  ## predicciones en el test set expresadas como probabilidad
...
predicciones.base.datos <- predict(adaboostfit, newdata = df.base.datos, type="prob" , na.action = na.pass)  ## predicciones en la base de datos expresadas como probabilidad
...
write.xlsx(x= predicciones.base.datos, file= "Screening por Adaboosting con caret.xlsx" , colNames= TRUE, keepNA=TRUE) # funcion para guardar los resultados del screening en la base de datos
