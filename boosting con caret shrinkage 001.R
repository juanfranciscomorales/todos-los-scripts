...
gbmGrid.001 <-  expand.grid(  ## con esto lo que voy a hacer es decir el barrido que va a hacer la funcion para optimizar los siguientes parámetros de gbm
        
        interaction.depth = 1 , ## este parametro es para ver la profundidad del arbol optima
        
        n.trees = seq(from = 1000 , to = 15000, by = 1000), ## este parametro es para ver el numero optimo de arboles
        
        shrinkage = 0.001 , ## es un valor de restriccion. Tengo que buscar el valor optimo. Mientras mas bajo mejor pero el costo computacional es mayor
...
gbmfit.001 <- train(clase ~ .,## uso la funcion train del paquete caret para hacer knn. en esta linea especifico cual es el valor a predecir y cuales son las variables independientes. 
                    
                    data = training, ## le digo cuales son mis datos para armar el modelo
                    
                    method = "gbm", ## aca le digo que use knn para armar el modelo
                    
                    trControl = ctrl.001,  ## le digo que use el elemento ctrl para optimizar el modelo
                    
                    tuneGrid = gbmGrid.001 , ## hago pasar el elemento gbmGrid para probar y encontrar cuales son los valores optimos 
...
gbmfit.001 ## imprimo el resultado

plot(gbmfit.001) ## grafico los resultados del cross validation



variables.importantes <- head(summary(gbmfit.001), n = 20) ## extraigo un data frame que contiene los valores de influencia y el nombre de las variables. con n decido cuantas variables mas importantes extraigo
...
predicciones.train <- predict(gbmfit.001, newdata = training , type = "prob" , na.action = na.pass) ## hago la prediccion en el training set obteniendo los resultados como probabilidad
...
predicciones.test <- predict(gbmfit.001, newdata = test, type="prob" , na.action = na.pass)  ## predicciones en el test set expresadas como probabilidad
...
resultado.gbm.001 <- list("Modelo armado por Boosting", gbmfit.001  , "AUC ROC Training" , auc.training, "AUC ROC Test", auc.test) ## armo una lista con todos los resultados que quiero que se impriman
...
gbmGrid <-  expand.grid(  ## con esto lo que voy a hacer es decir el barrido que va a hacer la funcion para optimizar los siguientes parámetros de gbm
        ...
        n.trees = seq(from = gbmfit.001$finalModel$n.trees -999 , to = gbmfit.001$finalModel$n.trees + 999 , by = 1), ## este parametro es para ver el numero optimo de arboles
...
resultado.gbm <- list("Modelo armado por Boosting", gbmfit  , "AUC ROC Training" , auc.training,"Resultado K-fold CV" , getTrainPerf(gbmfit) , "AUC ROC Test", auc.test) ## armo una lista con todos los resultados que quiero que se impriman
...
predicciones.train <- predict(gbmfit, newdata = training, type="prob" , na.action = na.pass)  ## predicciones en el train set expresadas como probabilidad
...
saveRDS(gbmfit, "gbmfit-001.rds") ## guardo el modelo 


gbmfit <- readRDS("gbmfit-001.rds") ## vuelvo a cargar el modelo
