...
clasificaciones.test.set.lm <- function (test.set = "Dtest.xlsx", lista.de.modelos = lista.modelos){
        ...
        for ( i in 1:length(lista.predicciones.test)) {
                lista.resultados.test.inactivos[[i]]<-   lista.predicciones.test[[i]] < puntos.corte.ROC.lm$cutoff[[i]]    
...
tabla.BC.test.set<-clasificaciones.test.set.lm(test.set = "Dtest.xlsx", lista.de.modelos = lista.modelos)