...
df <- clasificaciones.test.set.ensemble.minimo.lm2(test.set  = "dudes75.csv",cant.modelos = 12, x = tabla.AUC.ordenadas.test.set)  ## aplico la funcion anterior para obtener un data frame con los valores de score minimo y la clase real
...
curva.roc.dude <- roc(predictor = df$minimo ,response = df$clase, direction="<" , ci = TRUE , auc = TRUE , conf.level=0.95 , ci.method = "delong", reuse.auc=TRUE , plot =  TRUE) ## calculo la curva ROC con los datos del score minimo y la clase verdadera
...
predicciones <- ifelse( df$minimo > resultados.ensemble.minimo[[7]], yes = 1,no = 0) ## predicciones aplicando el ensemble de operador minimo y usando el punto de corte que obtuve con el training
...
df <- clasificaciones.test.set.ensemble.minimo.lm2(test.set  = "Descriptores DUDE completa sofi.csv",cant.modelos = j, x = tabla.AUC.ordenadas.test.set)  ## aplico la funcion anterior para obtener un data frame con los valores de score minimo y la clase real
