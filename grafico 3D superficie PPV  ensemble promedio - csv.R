...
df <- clasificaciones.test.set.ensemble.promedio.lm2(test.set  = "Dudepoliaminas2016.csv",cant.modelos = 15, x = tabla.AUC.ordenadas.test.set)  ## aplico la funcion anterior para obtener un data frame con los valores de score promedio y la clase real
...
predicciones <- ifelse( df$promedio > resultados.ensemble.promedio[[6]], yes = 1,no = 0) ## predicciones aplicando el ensemble de operador promedio y usando el punto de corte que obtuve con el training
