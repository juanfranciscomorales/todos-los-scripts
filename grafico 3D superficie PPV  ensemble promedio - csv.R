...
df <- clasificaciones.test.set.ensemble.promedio.lm2(test.set  = "Descriptores DUDE 1 Sofi.csv",cant.modelos = 20, x = tabla.AUC.ordenadas.test.set)  ## aplico la funcion anterior para obtener un data frame con los valores de score promedio y la clase real
...
curva.roc.dude <- roc(predictor = df$promedio ,response = df$clase, direction="<") ## calculo la curva ROC con los datos del score promedio y la clase verdadera
