...
resultado.rf <- list("Modelo armado por Random Forest", rf ,"Numero de Árboles Óptimo", cant.arboles.optimo , "AUC ROC Training" , auc.training, "OOB error" , rf$err.rate[ cant.arboles.optimo , 1] ,"AUC ROC Test", auc.test) ## armo una lista con todos los resultados que quiero que se impriman
...
predicciones.test <- as.data.frame(predict(rf, newdata = test, type="prob" , na.action = na.pass))  ## predicciones en el test set expresadas como probabilidad
